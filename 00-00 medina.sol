...
contract Payroll {
    ...
    function addEmployee(
        ...
        if (isMarried) {
            ...
            socialContributions = (netSalary * 5275) / 100000; 
        }
...
receive() exists?  fallback()
    ...
    */
    ...
    function sendViaTransfer(address payable employeeAddress) external onlyCompany payable {
    ...
    uint256 taxRate = employee.taxRate;

    uint256 tax = (salary * taxRate) / 100;
    uint256 netSalary = salary - tax;
    
    require(address(this).balance >= netSalary, "Insufficient contract balance to pay the employee");
    
    employee.netSalary = netSalary;
    employee.paid = true;
    totalTaxPayments += tax;

    employeeAddress.transfer(netSalary); // Paying the employee
    taxInstitution.transfer(tax); // Sending the tax to the institution
